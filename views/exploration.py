...
class Explore(arcade.View):
    ...
    def on_draw(self):
        ...
        arcade.draw_text(str(state.state.player.pos.tuple()), center_screen.x, center_screen.y + (state.state.cell_size.y * .37),
        ...
        for x_off, y_off in state.state.generate_radius(state.state.render_radius):
            ...
            if tile := state.state.grid.get(*real_grid_pos):
                render_queue.append(
                    (tile, render_pos, render_pos + (-(state.state.cell_size.x / 2), state.state.cell_size.y / 2), state.state.cell_size))
            else:
                arcade.draw_rectangle_outline(render_pos.x, render_pos.y, state.state.cell_size.x - 2, state.state.cell_size.y - 2,
