...
class MovementAnimator(arcade.View):
    def __init__(self, grid_start, grid_end, animation_steps):
        super().__init__()
        self.render_radius = state.render_radius
    ...
    def get_affected_tiles(self):
        ...
        for offset in state.generate_radius(self.render_radius):
            tile_pos.update((self.grid_end + offset, self.grid_start + offset))
        return [
            (tile, ((pos - self.grid_start) * state.cell_size) + center, ((pos - self.grid_end) * state.cell_size) + center)
            for pos in tile_pos
            if (tile := state.grid.get(*pos))
        ]
    ...
    def get_empty_tiles(self):
        ...
        return [
            (
                ((pos - self.grid_start) * state.cell_size) + center,
                ((pos - self.grid_end) * state.cell_size) + center
            )
            for pos in tile_pos
            if state.grid.get(*pos) is None
        ]
    ...
    def update(self, delta_time: float):
        ...
        if self.current_steps > self.animation_steps:
            ...
            state.window.show_view(exploration.Explore())
    ...
    def on_draw(self):
        ...
        arcade.draw_text(str(state.player.pos.tuple()), center.x, center.y + (state.cell_size.y * .37), arcade.color.LIGHT_GRAY,
